...
async function deleteSportReceiptsFromCloud(receiptIds) {
    ...
    try {
        ...
        if (error) {
            console.warn("Не удалось удалить чеки Sport Bar из базы. Локально они скрыты:", error);
            return false;
        }
        return true;
    } catch (error) {
        console.warn("Ошибка удаления чеков Sport Bar из базы. Локально они скрыты:", error);
        return false;
    }
}
        ...
        if (false) {
            try {
                await supabaseClient
                    .from("receipts_history")
                    .update({ shift_ended_at: endedAtISO })
                    .eq("shift_id", closingShift.id);
            } catch (error) {
                console.warn("Не удалось обновить время окончания смены в базе:", error);
            }
        }
...
async function closeCurrentReceipt() {
    ...
    currentSystem === "cafe"
        ...
        if (SYSTEM_CONFIG[currentSystem].cloudHistory && currentSystem !== "sport") {
            let { error } = await supabaseClient.from(historyTable).insert([{
                ...
                bartender_name: closedReceipt.bartenderName || "",
                shift_id: closedReceipt.shiftId || "",
                shift_started_at: closedReceipt.shiftStartedAtISO || null,
                shift_ended_at: closedReceipt.shiftEndedAtISO || null,
                business_date: closedReceipt.businessDateKey || null,
...
async function clearHistory() {
    ...
    if (currentSystem === "sport") {
        ...
        showConfirm(`Очистить чеки за ${shiftLabel}?`, async () => {
            ...
            await deleteSportReceiptsFromCloud(ids);
...
window.deleteHistoryReceipt = async function (receiptId) {
    ...
    showConfirm("Удалить этот чек из истории?", async () => {
        ...
        if (currentSystem === "sport") {
            await deleteSportReceiptsFromCloud([receiptId]);
