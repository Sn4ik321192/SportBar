...
if (prevMonthBtn) {
    prevMonthBtn.addEventListener("click", async () => {
        ...
        await loadHistoryFromCloud(
            historyViewDate.getFullYear(),
            historyViewDate.getMonth()
        );
    });
}
...
function updateHistoryMonthTitle() {
    ...
    const monthName = historyViewDate.toLocaleString("ru-RU", {
        month: "long",
        year: "numeric"
    });
...
async function loadHistoryFromCloud(year, month) {
    ...
    if (error) {
        console.error("Ошибка загрузки истории из базы:", error);
        showAlert("Не удалось загрузить историю из базы", "Ошибка");
        return;
    }

    receiptHistory = (data || []).map(item => ({
        id: item.id,
...
function renderMenu() {
    ...
    categoriesToShow.forEach((category, categoryIndex) => {
        ...
        if (category.category === "Мясо, закуски") {
            ...
            note.textContent = "За орешки и гренки +25 лей сверху.";
...
// добавляем в историю на сайте
receiptHistory.unshift(closedReceipt);
saveHistory();
renderHistory();
...
<button class="danger-btn history-delete-one-btn" onclick="event.stopPropagation(); deleteHistoryReceipt('${item.id}')">
    Удалить чек
</button>
</div>
            </div>
...
window.deleteHistoryReceipt = async function (receiptId) {
    ...
    showConfirm("Удалить этот чек из истории?", async () => {
        ...
        receiptHistory = receiptHistory.filter(item => item.id !== receiptId);
        saveHistory();
        renderHistory();
